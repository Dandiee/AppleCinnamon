...
namespace AppleCinnamon.Chunks
{
    public static class SkyDome
    {
        ...
        public static IEnumerable<VertexSkyBox> GenerateSkyDome()
        {
            ...
            var scaler = new Vector3(100);
            var offset = new Vector3(0, 135, 0);
            for (var i = 0; i < 6; i++)
            {
        ...
        public static IEnumerable<VertexSkyBox> GenerateSkyDome1()
        {
            ...
            var offset = new Vector3(0, 150, 0);

            for (var i = 0; i < horizontalSteps; i++)
            {
                for (var j = 0; j < Resolution; j++)
                {
                    var v1 = startVector.Rotate(Vector3.UnitX, (j + 0) * step).Rotate(Vector3.UnitY, (i + 0) * step) + offset;
                    var v2 = startVector.Rotate(Vector3.UnitX, (j + 1) * step).Rotate(Vector3.UnitY, (i + 0) * step) + offset;
                    var v3 = startVector.Rotate(Vector3.UnitX, (j + 1) * step).Rotate(Vector3.UnitY, (i + 1) * step) + offset;
                    var v4 = startVector.Rotate(Vector3.UnitX, (j + 0) * step).Rotate(Vector3.UnitY, (i + 1) * step) + offset;
                    ...
                    yield return new VertexSkyBox(v1, direction, new Vector2(0, 0));
                    yield return new VertexSkyBox(v2, direction, new Vector2(0, 1));
                    yield return new VertexSkyBox(v3, direction, new Vector2(1, 1));

                    yield return new VertexSkyBox(v1, direction, new Vector2(0, 1));
                    yield return new VertexSkyBox(v3, direction, new Vector2(1, 1));
                    yield return new VertexSkyBox(v4, direction, new Vector2(1, 0));
